...
class SnakeGame(gym.Env):
    ...
    def get_new_relative_state(self):
        ...
        for agent in self.agents:
            ...
            closest = self.fruits[np.argmin([math.dist(f, agent.head) for f in self.fruits])]
            x, y = agent.head
            bodies_right = sum([1 for b in other_snake_bodies if 0 < b[0] - x < 3 and math.dist(b, agent.head) < 5]) > 0
            bodies_left = sum([1 for b in other_snake_bodies if 0 < x - b[0] < 3 and math.dist(b, agent.head) < 5]) > 0
            bodies_down = sum([1 for b in other_snake_bodies if 0 < b[1] - y < 3 and math.dist(b, agent.head) < 5]) > 0
            bodies_up = sum([1 for b in other_snake_bodies if 0 < y - b[1] < 3 and math.dist(b, agent.head) < 5]) > 0
            state = (
                int(closest[0] > x),
                int(closest[0] < x),
                int(closest[1] > y),
