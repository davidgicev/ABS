...
class SnakeGame(gym.Env):
    ...
    def step(self, actions):
        ...
        return self.get_povs(), rewards, done, {}
    ...
    #         state = (
    #             closest[0] > agent.head[0],
    #             closest[0] < agent.head[0],
    #             closest[1] > agent.head[1],
    #             closest[1] < agent.head[1],
    #             agent.head[0] == 0,
    #             agent.head[1] == 0,
    #             agent.head[0] == self.width - 1,
    #             agent.head[1] == self.height - 1,
    #             agent.direction == 0,
    #             agent.direction == 1,
    #             agent.direction == 2,
    #             agent.direction == 3,
    ...
    def get_povs(self):
        ...
        for a, agent in enumerate(self.agents):
            ...
            for i in range(2):
                for j in range(3):
                    x = hx + vx[0]*i + vy[0]*(1-j)
                    y = hy + vx[1]*i + vy[1]*(1-j)
                    if x < 0 or x >= self.width or y < 0 or y >= self.height:
                        pov[i][j] = 4
