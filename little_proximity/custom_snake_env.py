...
class SnakeGame(gym.Env):
    ...
    def get_new_relative_state(self):
        ...
        for agent in self.agents:
            ...
            closest = self.fruits[np.argmin([math.dist(f, agent.head) for f in self.fruits])]
            x, y = agent.head
            bodies_right = (x + 1, y) in other_snake_bodies
            bodies_left = (x - 1, y) in other_snake_bodies
            bodies_down = (x, y + 1) in other_snake_bodies
            bodies_up = (x, y - 1) in other_snake_bodies
            state = (
                int(closest[0] > x),
                int(closest[0] < x),
                int(closest[1] > y),
